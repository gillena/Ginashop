...
class ProductModel extends BaseModel
{
    ...
    function getProdRecently()
    {
        return $this->db->table($this->tableName())
                        ->select('id, title, slug, thumb, price, totalRatings, quantity, discount, totalUserRatings, vr_model_url')
                        ->where('status', '=', 1)
                        ->orderBy('view')
                        ->limit(12)
                        ->get();
    ...
    {
        $data = $this->db->table($this->tableName())
                        ->select('id, slug, thumb, title, create_At, price, sold, quantity, vr_model_url')
                        ->orderBy('sold')
                        ->limit(4)
                        ->get();
        return $data;
    ...
    {
        return $this->db->table('ratings r')
                        ->select('r.id, r.create_at, p.title, r.status, r.star, r.comment, u.fullname, u.avatar')
                        ->join('user u', 'r.user_id = u.id')
                        ->join('product p', 'p.id = r.prod_id')
    ...
    {
        return $this->db->table('ratings r')
                        ->select('r.id, r.create_at, r.star, r.comment, u.fullname, u.avatar')
                        ->join('user u', 'r.user_id = u.id')
                        ->where('r.prod_id', '=', $prod_id)
                        ->where('r.status', '=', 1)
                        ->get();
    ...
    // Thêm phương thức để làm việc với trường vr_model_url
    function updateVRModelUrl($id, $url)
    {
        return $this->db->findByIdAndUpdate($this->tableName(), $id, ['vr_model_url' => $url]);
    }
}
