...
class Account extends Controller
{
    ...
    function registerApi()
    {
        ...
        $body = 'Click vào đây để hoàn thành đăng ký của bạn: <a href="' . $baseUrl . '/signup-comfirm/' . $token . '">Xác nhận</a> Đường dẫn sẽ hết hạn trong 15 phút.';
    ...
    function forgotPasswordApi()
    {
        ...
        $baseUrl = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['SCRIPT_NAME']);
        $body = 'Click vào đây để xác nhận email của bạn: <a href="' . $baseUrl . '/account/finalForgotPassword/' . $token . '">Xác nhận</a> Đường dẫn sẽ hết hạn trong 15 phút.';
    ...
    function updateUserCurrent()
{
    ...
    // Kiểm tra nếu người dùng nhập mật khẩu mới thì xác thực
    if (!empty($dataPost['new_password'])) {
        $this->req->rules([
        ...
        // Set message cho mật khẩu
        $this->req->message([
            'old_password.required' => 'Vui lòng không để trống mật khẩu cũ.',
            'new_password.required' => 'Vui lòng không để trống mật khẩu mới.',
    ...
    if ($dataUserUp['email'] != $dataPost['email'] && !empty($checkEmail)) {
        return $this->res->setToastSession('error', 'Email đã tồn tại', 'my-account');
    }

    $checkPhone = $this->userModel->checkPhoneExisted($dataPost['phone']);
    if ($dataUserUp['phone'] != $dataPost['phone'] && !empty($checkPhone)) {
        return $this->res->setToastSession('error', 'Số điện thoại đã tồn tại', 'my-account');
    }

    // Kiểm tra lại mật khẩu nếu người dùng nhập mật khẩu mới
    if (!empty($dataPost['new_password']) && !password_verify($dataPost['old_password'], $dataUserUp['password'])) {
        return $this->res->setToastSession('error', 'Mật khẩu cũ không chính xác.', 'my-account');
    }

    // Tạo mật khẩu mới nếu có
    $hashedPassword = !empty($dataPost['new_password']) ? password_hash($dataPost['new_password'], PASSWORD_BCRYPT) : $dataUserUp['password'];
    ...
    // Kiểm tra và cập nhật địa chỉ nếu có
    if (!empty($dataPost['address'])) {
        $dataUpdate['address'] = $dataPost['address'];
    }
    ...
    if (!empty($avatar['name'])) {
        ...
        if (!Format::validateUploadImage($avatar)) {
            return $this->res->setToastSession('error', 'Có lỗi về ảnh vui lòng kiểm tra lại.', 'my-account');
        }
    ...
    if ($updateUser) {
        return $this->res->setToastSession('success', 'Cập nhật tài khoản thành công.', 'my-account');
    } else {
        return $this->res->setToastSession('error', 'Cập nhật tài khoản thất bại.', 'my-account');
    }
}
