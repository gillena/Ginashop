...
class Checkout extends Controller
{
    ...
    private function handlePaymentMethod($dataInsertOrder, $dataCartNew, $payment_method_id, $dataGet = [])
    {
        ...
        foreach ($dataCartNew as $orderItem) {
            ...
            $email = $this->userModel->getUserEmail($this->user_id);

            // Gửi email xác nhận đơn hàng
            $emailSent = Services::sendOrderConfirmationEmail($email, $dataInsertOrder['order_code'], $dataInsertOrder['total_money'] , $dataCartNew);
            if (!$emailSent) {
                // Thêm địa chỉ email vào thông báo
                return $this->res->setToastSession('error', 'Gửi email xác nhận thất bại cho email: ' . $email . '. Vui lòng thử lại.', 'checkout');
            }
    ...
    function updateOrderStatus()
    {
        ...
        return $this->res->setToastSession('success', 'Bạn đã Cập nhật đơn hàng thành công.', 'my-account');
    }
}
