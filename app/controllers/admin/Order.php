...
class Order extends Controller
{
    ...
    function updateOrderStatus()
    {
        ...
        if ($updateStatus) {
            return $this->res->setToastSession('success', 'Bạn đã Cập nhật đơn hàng thành công.', 'admin/order-detail/' . $dataPost['idData']);
        } else {
            return $this->res->setToastSession('error', 'Có lỗi xảy ra vui lòng thử lại', 'admin/order-detail/' . $dataPost['idData']);
        }
    }
    ...
    function updatePaymentMethod($id)
    {
        ...
        if ($updatePaymentMethod) {
            return $this->res->setToastSession('success', 'Cập nhật thành công.', 'admin/payment-method');;
        } else {
    ...
    {
        if (!$this->req->isPost()) {
            return $this->res->setToastSession('error', 'Có lỗi vui lòng thử lại.', 'admin/payment-method');
        }
        $dataPost = $this->req->getFields();

        $success = $this->paymentModel->deletePaymentMethod($dataPost['id']);

        if (!$success) {
            return $this->res->setToastSession('error', 'Xoá thất bại.', 'admin/payment-method');
        }

        return $this->res->setToastSession('success', 'Xoá thành công.', 'admin/payment-method');
    }
