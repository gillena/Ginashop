...
class DashBoard extends Controller
{
    ...
    function storeCustom()
    {
        ...
        //Get image
        $logo = $_FILES['logo'] ?? '';
        $backgroundImage = $_FILES['background_image'] ?? ''; 
        ...
        // Neu co loi validate se hien thi loi
        if (!empty($dataError)) {
        ...
        if (!empty($backgroundImage['name'])) {
            // Validate Upload image background
            if (!Format::validateUploadImage($backgroundImage)) {
                $this->Toast('error', 'Kiểm tra lại file upload background.');
                return $this->renderStoreCustomPage($dataBanner, $dataStoreCustom, $dataCate);
            }

            // Upload ảnh background lên cloudinary
            $urlBackgroundImage = Services::uploadImageToCloudinary($backgroundImage['tmp_name']);
            if (empty($urlBackgroundImage)) {
                $this->Toast('error', 'Upload ảnh background thất bại.');
                return $this->renderStoreCustomPage($dataBanner, $dataStoreCustom, $dataCate);
            }
            $dataStoreCustomUpdate['background_image'] = $urlBackgroundImage; 
        }
