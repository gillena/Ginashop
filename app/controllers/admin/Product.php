...
class Product extends Controller
{
    ...
    function addProduct()
    {
        ...
        $dataInsertProd = [
            ...
            'vr_model_url' => $dataPost['vr_model_url'] ?? null,
        ];
    ...
    function updateProduct($id)
    {
        ...
        if (empty($updateProd)) {
            $this->Toast('error', 'Có lỗi khi Cập nhật sản phẩm.');
            return $this->renderUpdatePage($cateData, $brandData, $prodImages, $dataProd);
        }
        ...
        // Thanh cong se sang trang danh sach san pham
        return $this->res->setToastSession('success', 'Cập nhật sản phẩm thành công.', 'admin/product');
    }
    ...
    function productVariants($id)
    {
        ...
        foreach ($dataUpdateVariants as $value) {
            ...
            if (!$updateProductVariant) {
                return $this->res->setToastSession('error', 'Có lỗi trong quá trình Cập nhật.', 'admin/product-variants/' . $id);
            }
        }

        return $this->res->setToastSession('success', 'Cập nhật thành công.', 'admin/product-variants/' . $id);
    }
    ...
    {
        if (!$this->req->isPost()) {
            echo $this->res->dataApi('400', 'Cập nhật thất bại.', []);
            return;
        }
        ...
        echo $this->res->dataApi('200', 'Cập nhật thành công.', []);
        return;
    }
