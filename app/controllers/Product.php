...
class Product extends Controller
{
    ...
    function filterProduct()
    {
        ...
        // Kiểm tra có số sao không
        if (!empty($dataPost['rating'])) {
            $rating = intval($dataPost['rating']);
            if ($rating >= 1 && $rating <= 5) { // Đảm bảo giá trị rating hợp lệ
                $conditions[] = "totalRatings >= $rating"; // Thay đổi điều kiện theo yêu cầu
            }
        }

        // Kiểm tra có số sao không
             if (!empty($dataPost['rating1'])) {
            $conditions[] = "totalRatings >= " . intval($dataPost['rating1']);
        }

        // Kiểm tra có khuyến mãi không
        if (!empty($dataPost['promotion']) && $dataPost['promotion'] == 1) {
            $conditions[] = "discount > 0"; // Điều kiện lọc sản phẩm có khuyến mãi
        }
