...
const toggleStatusProd = (id) => {
  ...
  const getVariant = (id) => {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.code == 400) {
          window.location.href = '/GINA_SHOP/';
        }
      },
      error: function (xhr, status, error) {
        console.error(error);
                      ...
                      <div class="actions-hover">
                          <ul class="action-list mb-0">
                              ...
                              <li class="select-option">
                                  ${
                                    quantity > 0
                                      ? `<a href="${productLink}" class="btn-action-lagre">Xem chi tiết</a>`
  ...
  const filterProduct = async () => {
    ...
    const newURL = `http://localhost/GINA_SHOP/product-category?${queryString}`;
